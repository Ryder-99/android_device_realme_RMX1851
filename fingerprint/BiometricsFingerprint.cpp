...
Return<RequestStatus> BiometricsFingerprint::authenticateAsType(uint64_t operationId, uint32_t gid, uint32_t fingerprintAuthType) {
    return OppoToAOSPRequestStatus(mOppoBiometricsFingerprint->authenticateAsType(operationId, gid, fingerprintAuthType));
}
...
Return<RequestStatus> BiometricsFingerprint::setScreenState(uint32_t screen_state) {
    return OppoToAOSPRequestStatus(mOppoBiometricsFingerprint->setScreenState(screen_state));
}
...
Return<RequestStatus> BiometricsFingerprint::sendFingerprintCmd(uint32_t cmdId,
    const hidl_vec<uint8_t>& in_buf) {
    return OppoToAOSPRequestStatus(mOppoBiometricsFingerprint->sendFingerprintCmd(cmdId, in_buf));
}

Return<RequestStatus> BiometricsFingerprint::setFingerKeymode(uint32_t enable) {
    return OppoToAOSPRequestStatus(mOppoBiometricsFingerprint->setFingerKeymode(enable));
}
